                    ...
                    <div class="card-content">
                        ...
                        <div class="table-responsive">
                            <table id="datatables" class="table table-bordered table-striped">
                                ...
                                @endphp
                                <tbody>   
                                    @foreach ($rating_tukang as $data_tukang)
                                        <tr>
                                            <td class="text-center">{{ $no++ }}</td>
                                            <td>{{ $data_tukang->nama_tukang }} ({{ $data_tukang->id_tukang }})</td>
                                            <td>{{ $data_tukang->kriteria_1 }}</td>
                                            <td>{{ $data_tukang->kriteria_2 }}</td>
                                            <td>{{ $data_tukang->kriteria_3 }}</td>
                                            <td>{{ $data_tukang->kriteria_4 }}</td>
                                            <td class="td-actions text-center">
                                                <a type="button" rel="tooltip" data-placement="bottom" title="Edit Data" class="btn btn-success" href="/data_rating/edit/{{ $data_tukang->id_rating }}"><i class="material-icons">edit</i></a>
                                                <a type="button" rel="tooltip" data-placement="bottom" title="Hapus Data" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?')" href="/data_rating/delete/{{ $data_tukang->id_rating }}"><i class="material-icons">close</i></a>
                                            </td>
                                    </form>
                                </td>
        ...
        <div class="modal-body">
            ...
            <form method="post" action="/data_rating/store" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <select class="selectpicker" data-style="btn btn-primary btn-round" title="Pilih tukang" name="id_tukang">
                        @foreach ($tukang as $tukang)
                            <option value="{{ $tukang->id_tukang }}">{{ $tukang->nama_tukang }} ({{ $tukang->id_tukang }})</option>
                        @endforeach
                    </select>
                    </div>
                  </div>
