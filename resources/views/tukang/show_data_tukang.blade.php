          ...
          <div class="card card-profile">
            <div class="card-avatar">
              <a href="javascript:;">
                ...
                <img class="img" src="{{ url('img_tukang/'.$data->foto_tukang) }}" style="width: 130px; height: 130px;" />
              </a>
            </div>
          ...
          <div class="card">
            ...
            <div class="card-content">
              ...
              <div class="card-content table-responsive table-full-width" style="margin-left: 50px">
                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary ">{{ __('ID tukang') }}</label>
                  <div class="col-sm-8">{{ $data->id_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Nama tukang') }}</label>
                  <div class="col-sm-8">{{ $data->nama_tukang }}</div>
                </div>
                
                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Email') }}</label>
                  <div class="col-sm-8">{{ $data->email_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Password') }}</label>
                  <div class="col-sm-8">{{ $data->password_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('No. Telp') }}</label>
                  <div class="col-sm-8">{{ $data->telp_tukang }}</div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-8">{{ $data->latitude_tukang }}</div>
                </div>
      
                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Longitude') }}</label>
                  <div class="col-sm-8">{{ $data->longitude_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Alamat') }}</label>
                  <div class="col-sm-8">{{ $data->alamat_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Spesifikasi tukang') }}</label>
                  <div class="col-sm-8">{{ $data->spesifikasi_tukang }}</div>
                </div>

                <div class="row">
                  <label class="col-sm-4 col-form-label text-primary">{{ __('Jangkauan Kategori tukang') }}</label>
                  <div class="col-sm-8">{{ $data->jangkauan_kategori_tukang }}</div>
                </div>
                ...
                <div class="row">
                  @foreach ($kategori_tukang as $kategori_tukang)
                  
                    <div class="col-sm-8">
                      <li>
                        {{ $kategori_tukang->nama_kategori }}
                      </li>
                    </div>
                  @endforeach
                </div> --}}
              ...
              <div class="form-footer text-right" style="margin-bottom: 60px">
                <a type="button" class="btn btn-primary pull-right" href="/data_tukang/edit/{{ $data->id_tukang }}">Update Data</a>
                <a type="button" class="btn btn-white pull-right" href="/data_tukang">Kembali</a>
              </div>

              {{-- <a type="button" class="btn btn-primary float-right" href="/data_tukang/edit/{{ $data->id_tukang }}">Update Data</a> --}}
          ...
          <div class="card">
              ...
              <div class="nav-tabs-navigation">
                <div class="nav-tabs-wrapper">
                  ...
                  <ul class="nav nav-tabs justify-content-end" data-tabs="tabs">
                    <li class="nav-item">
                      <a type="button" rel="tooltip" data-placement="bottom" title="Tambah Data" class="nav-link active" href="/data_tukang/show/kategori/add/{{ $data->id_tukang }}"><i class="material-icons">add</i> Tambah Data</a>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div class="card-header card-header-tabs" data-background-color="purple">
              ...
              <div class="nav nav-tabs" data-tabs="tabs">
                  ...
                  <a type="button" class="btn btn-primary pull-right " href="/data_tukang/show/kategori/add/{{ $data->id_tukang }}"><i class="material-icons">add</i> Tambah Data</a>
              </div>
          </div>
                        ...
                        @foreach ($kategori_tukang as $kategori)
                            <tr>
                                ...
                                <td class="td-actions text-center">
                                  <a type="button" rel="tooltip" data-placement="bottom" title="Lihat Data" class="btn btn-info" href="/data_tukang/show/ukuran/{{ $kategori->id_detail_kategori }}"><i class="material-icons">trending_flat</i></a>
                                  <a type="button" rel="tooltip" data-placement="bottom" title="Edit Data" class="btn btn-success" href="/data_tukang/show/kategori/edit/{{ $kategori->id_detail_kategori }}"><i class="material-icons">edit</i></a>
                                  <a type="button" rel="tooltip" data-placement="bottom" title="Hapus Data" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?')" href="/data_tukang/show/kategori/delete/{{ $kategori->id_detail_kategori }}"><i class="material-icons">close</i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
